...
class lessc
{
    ...
    protected static $operatorString; // regex string to match any of the operators

    // types that have delayed computation
    protected static $dtypes = array('expression', 'variable',
    ...
    public function expressionList(&$exps)
    {
        $values = array();
    ...
    // a single expression
    public function expression(&$out)
    {
        $s = $this->seek();
        $needWhite = true;
    ...
    public function expHelper($lhs, $minP, $needWhite = true)
    {
        $ss = $this->seek();
    ...
    public function propertyValue(&$value)
    {
        $values = array();
    ...
    // an import statement
    public function import(&$url, &$media)
    {
        $s = $this->seek();
    ...
    public function mediaTypes(&$types, &$rest)
    {
        $s = $this->seek();
        $types = array();
    ...
    // a string
    public function string(&$string, &$d = null)
    {
        $s = $this->seek();
    ...
    // a numerical unit
    public function unit(&$unit, $allowed = null)
    {
    ...
    public function argumentValues(&$args, $delim = ';')
    {
        $s = $this->seek();
        if (!$this->literal('(')) {
            return false;
    ...
    public function argumentDef(&$args, $delim = ';')
    {
        $s = $this->seek();
        if (!$this->literal('(')) {
            return false;
    ...
    public function tags(&$tags, $simple = false, $delim = ',')
    {
        $tags = array();
    ...
    public function tagBracket(&$value)
    {
        $s = $this->seek();
    ...
    // a single tag
    public function tag(&$tag, $simple = false)
    {
        if ($simple) {
            $chars = '^,:;{}\][>\(\) ';
        } else {
    ...
    public function compressList($items, $delim)
    {
        if (count($items) == 1) {
            return $items[0];
        } else {
     ...
     *
     */
    public function compileBlock($block, $parentTags = null, $bindEnv = true)
    {
        $children = array();
        ...
        if (isset($block['__args'])) {
            ...
            foreach ($block['__args'] as $arg) {
                if (isset($arg[1])) {
                    $this->append($this->vPrefix.$arg[0], $arg[1]);
                }
            }
        }
    ...
    public function indent($str, $level = null)
    {
        if (is_null($level)) {
    ...
    public function compileProperty($name, $value, $level = 0)
    {
    ...
    public function lib_rgbahex($arg)
    {
    ...
    public function lib_quote($arg)
    {
        return '"'.$this->compileValue($arg).'"';
    }

    public function lib_unquote($arg)
    {
    ...
    public function lib_floor($arg)
    {
        $arg = $this->reduce($arg);
        return floor($arg[1]);
    }

    public function lib_round($arg)
    {
        $arg = $this->reduce($arg);
        return round($arg[1]);
    }
    ...
    // todo: add hsl
    public function funcToColor($func)
    {
        $fname = $func[1];
    ...
    public function reduceBlock($block)
    {
        if (isset($block['__args'])) {
            $this->push();
            foreach ($block['__args'] as $arg) {
    ...
    public function reduce($var, $defaultValue = array('number', 0))
    {
    ...
    public function evaluate($op, $left, $right)
    {
    ...
    public function op_number_color($op, $lft, $rgt)
    {
        if ($op == '+' || $op = '*') {
    ...
    public function op_color_number($op, $lft, $rgt)
    {
        if ($rgt[0] == '%') {
            $rgt[1] /= 100;
        }
    ...
    public function op_color_color($op, $left, $right)
    {
        $out = array('color');
    ...
    public function op_number_number($op, $left, $right)
    {
        if ($right[0] == '%') {
            $right[1] /= 100;
        }
    ...
    public function push($base = null)
    {
        $env = new stdclass;
        $env->parent = $this->env;
    ...
    public function setLiteral($value)
    {
    ...
    public function appendAll($name, $values)
    {
    ...
    public function getRaw($name, $failValue = false)
    {
        if (isset($this->env->store[$name])) {
    ...
    public function get($name, $top = null)
    {
    ...
    public function getVal($name, $skip = 0, $default = array('keyword', ''))
    {
    ...
    public function isProperty($name, $value, $isConcrete = true)
    {
    ...
    public function isBlock($name, $value, $isConcrete = true)
    {
        return is_array($value) && !array_key_exists(0, $value) &&
            substr($name, 0, 2) != '__' &&
    ...
    public function to($what, &$out, $until = false, $allowNewline = false)
    {
    ...
    public function peek($regex, &$out = null)
    {
        $r = '/'.$regex.'/Ais';
